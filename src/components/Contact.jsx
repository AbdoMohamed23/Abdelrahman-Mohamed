...
const Contact = () => {
  ...
  const [mailtoTriggered, setMailtoTriggered] = useState(false)
  ...
  const handleSubmit = (e) => {
    ...
    const mailtoLink = `mailto:${contactSettings.email}?subject=${subject}&body=${body}`

    // Open default email client
    window.location.href = mailtoLink

    // Reset form and show brief message
    setFormData({ name: '', email: '', subject: '', message: '' })
    setMailtoTriggered(true)
    setTimeout(() => setMailtoTriggered(false), 4000)
  }
            ...
            {mailtoTriggered && (
              <div className='mb-4 p-4 bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-700 rounded-lg'>
                <p className='text-green-800 dark:text-green-300 text-center font-medium'>
                  ✅ {i18n.language === 'ar' ? 'تم فتح بريدك الإلكتروني! أرسل الرسالة من هناك.' : 'Your email client has been opened. Please send the message from there.'}
                </p>
              </div>
            )}
